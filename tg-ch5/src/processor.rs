...
/// stride 调度用的大常数，pass = BigStride / priority
...
/// - `ready_queue`：就绪队列，stride 调度时每次取 stride 最小的进程
pub struct ProcManager {
    /// 所有进程实体的映射表
    tasks: BTreeMap<ProcId, Process>,
    /// 就绪队列（stride 调度：fetch 时选 stride 最小）
...
impl Schedule<ProcId> for ProcManager {
    ...
    /// 从就绪队列中取出 stride 最小的进程，并将其 stride += pass
    fn fetch(&mut self) -> Option<ProcId> {
        ...
        if let Some(p) = self.tasks.get_mut(&id) {
            let pass = BIG_STRIDE / p.priority.max(1);
            p.stride = p.stride.saturating_add(pass);
        }
        Some(id)
    }
}
