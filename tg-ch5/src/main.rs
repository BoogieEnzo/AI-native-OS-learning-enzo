...
mod impls {
    use crate::{
        build_flags, parse_flags, process::Process as ProcStruct, processor::ProcManager,
        Sv39, APPS, PROCESSOR, PROTAL_TRANSIT,
    };
    ...
    impl Scheduling for SyscallContext {
        ...
        /// set_priority 系统调用：设置当前进程优先级（stride 调度用）
        ///
        /// 要求 prio >= 2，否则返回 -1。
    ...
    impl Memory for SyscallContext {
        ...
        fn mmap(
            ...
            let flags = match parse_flags(unsafe {
                core::str::from_utf8_unchecked(&flags_buf)
            }) {
                Ok(f) => f,
                Err(_) => return -1,
            };
